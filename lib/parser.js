...
const matchAll = require('string.prototype.matchall') // node LTS 10 doesn't support string.matchAll
...
function parseChat (txt) {
  ...
  const chatItemRegexp = /(\d{1,2}:\d{1,2}:\d{1,2}) (\w*( ?\w{0,}(?= :))? ?:? ?)(.*\s?)/gm
  const dateOnNewLines = /(?<=\D)(?=\d{1,2}:\d{1,2}:\d{1,2} [\w ]*:? ?)/g
